...
#     f = netCDF4.Dataset(filename)
    ...
    lat = f['latitude'][:]
    phibot = f['phibot'][:]
    time = f['time'][:] # Days since 1992-01-01 00:00:00
...
#    phibot = np.subtract(phibot,phibot_spatial_mean) # Subtract mean value from 2d array
    ...
    # Get Mask
    masked_data = np.ma.masked_invalid(phibot[0,:,:])
    cmask = np.ma.getmask(masked_data) # Mask

    # Replace Masked Data with Fill Values
    phibot = np.ma.filled(masked_data,fill_value=0.)
