...
# loadfile_directory = ("../output/Grid_Files/nc/ATML/")  # Example 2 (time series)
# loadfile_directory = ("../temp/")  # Example 2 (time series)
...
# loadfile_prefix = ("convgf_atml") # Example 2 (time series)
...
frst_date = [2011,1,1,0,0,0]
last_date = [2023,1,1,0,0,0]
...
sta_file = ("../input/Station_Locations/POS_Select.txt")
...
#     lon1d = np.arange(0.,360.,2.)
    lat1d = np.arange(3.,54.,2.)
    lon1d = np.arange(73.,136.,2.)
...
for jj in range(0,numel):
    ...
    if (rank == 0):
        eamp,epha,namp,npha,vamp,vpha = load_convolution.main(grn_file,norm_flag,load_files,loadfile_format,regular,lslat,lslon,lsmask,lsmask_type,slat,slon,sname,cnv_out,
                                                              ldens)
    # For Worker Ranks, Run the Code But Don't Return Any Variables
    else:
        load_convolution.main(grn_file,norm_flag,load_files,loadfile_format,regular,lslat,lslon,lsmask,lsmask_type,\
            slat,slon,sname,cnv_out,ldens,rank,procN,comm)
